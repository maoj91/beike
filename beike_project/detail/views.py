...
from user.session_util import is_request_valid, get_wx_id
...
def sell_post_detail(request,offset):
    ...
    return render_to_response('sell_post_detail.html', {'post':post,'lat':lat,'lon':lon,'is_open':is_open,
        'image_list': image_list, 'image_num':image_num,
        'is_followed': is_followed, 'wx_id':wx_id, 'user_id':user_id,
        'is_owner': is_owner,'phone_checked':phone_checked,'email_checked':email_checked,
        'sms_checked':sms_checked,'phone':phone,
...
def sell_detail_save(request,offset):
    ...
    if request.method == 'POST':
        ...
        post.image_urls = get_image_info(request)
        post.save()
...
def buy_detail_edit(request,offset):
    ...
    user_image = ImageMetadata.deserialize_list(post.user.image_url)[0]
    categories = Category.objects.all();
    return render_to_response('buy_detail_edit.html', {'post':post,'lat':lat,'lon':lon,'is_open':is_open, 
        'is_followed': is_followed, 'wx_id':wx_id, 'is_owner': is_owner,'phone_checked':phone_checked,'email_checked':email_checked,'sms_checked':sms_checked,'phone':phone,
        'email':email,'user_image':user_image, 'categories':categories},RequestContext(request))
